...
def test(net, testLoader, criterion, epoch, args, logs):
    ...
    globals = torch.empty([samples, len(logs.keys)])
    locals = torch.empty([samples, len(logs.keys)])
...
if __name__=='__main__':
    ...
    args.save = os.path.join(datapath, 'Others/HMCNF')
    ...
    net = HMCNF.Net(len(keys1), len(keys2), len(keys3)).cuda()
