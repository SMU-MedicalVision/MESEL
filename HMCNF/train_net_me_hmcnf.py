...
def train(net, trainLoader, criterion, optimizer, epoch, args, logs):
    ...
    for batch_idx, (batch_data, batch_label) in enumerate(trainLoader):
        ...
        optimizer.zero_grad()
        batch_globals, batch_locals = net(batch_data)
        batch_loss = criterion(batch_globals, batch_label) + criterion(batch_locals, batch_label)
        batch_loss.backward()
        optimizer.step()
...
def test(net, testLoader, criterion, epoch, args, logs):
    ...
    globals = torch.empty([12, samples, len(logs.keys)])
    locals = torch.empty([12, samples, len(logs.keys)])
    ...
    with torch.no_grad():
        ...
        for batch_idx, (batch_data, batch_label) in enumerate(testLoader):
            ...
            globals[:, batch_idx * args.batch_size:batch_idx * args.batch_size + bs] = batch_globals.data.cpu()
            locals[:, batch_idx * args.batch_size:batch_idx * args.batch_size + bs] = batch_locals.data.cpu()

    test_loss = criterion(globals, labels).item() + criterion(locals, labels).item()
    logs.result.loc[epoch, 'test_loss'] = test_loss

    logits = (criterion.activate(globals) + criterion.activate(locals)) / 2
...
if __name__=='__main__':
    ...
    args.save = os.path.join(datapath, 'Others/Ensemble/HMCNF')
    ...
    net = HMCNF.MENet(len(keys1), len(keys2), len(keys3)).cuda()
    ...
    criterion = Loss.MultiExpertLoss(cls_num_list, N, prior)
