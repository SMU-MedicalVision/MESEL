...
def train(net, trainLoader, criterion, optimizer, epoch, args, logs):
    ...
    for batch_idx, (batch_data, batch_label) in enumerate(trainLoader):
        ...
        optimizer.zero_grad()
        batch_logits = net(batch_data, embedding.cuda())
...
if __name__=='__main__':
    ...
    args.save = os.path.join(datapath, 'Others/Single/MLGCN/{}'.format(args.t))
    ...
    net = MLGCN.Net(num_classes=len(keys), adj_matrix=prior - np.eye(len(keys)), t=args.t).cuda()
    ...
    criterion = Loss.BCELoss(cls_num_list, N, 'Sigmoid', prior)
