...
class LDM(nn.Module):
    ...
    def __init__(self, num_channels, num_classes):
        ...
        self.head_holistic = nn.Linear(num_channels, num_classes)
        self.head_divs = nn.ModuleList([self.make_head_div(num_channels) for i in range(num_classes)])
        self.rse = nn.Sequential(nn.Linear(num_classes, num_classes),
                                 nn.Sigmoid())

    def make_head_div(self, num_channels=128, feature_size=256):
        head_div = nn.Sequential(nn.Linear(num_channels, feature_size),
                                 NormalizeL2(),
                                 nn.ReLU(),
    ...
    def forward(self, x):
        ...
        embedding = self.head_holistic(x)

        division = torch.empty_like(embedding).to(device=embedding.device)
        for i, head_div in enumerate(self.head_divs):
            division[:, i] = torch.squeeze(head_div(x))

        prob_em = self.rse(torch.ones_like(embedding))
        prob_div = 1 - prob_em

        out = embedding * prob_em + division * prob_div
...
class Net(nn.Module):
    ...
    def __init__(self, num_classes=200, init_channels=12, growth_rate=16, base_channels=64,
        ...
        self.ldm = LDM(self.num_channels, num_classes)

    def forward(self, x):

        features = self.blocks(x)
        features = torch.squeeze(features)
        out = self.ldm(features)
...
class MENet(nn.Module):
    ...
    def __init__(self, num_classes=200, init_channels=12, growth_rate=16, base_channels=64,
        ...
        for expert in range(experts):
            ...
            projection_head.add_module("LDModule{}".format(expert), LDM(num_channels, num_classes))
