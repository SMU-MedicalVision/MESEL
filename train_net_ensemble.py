...
def test(net, testLoader, criterion, epoch, args, logs):
    ...
    labels = torch.empty([samples, len(keys)])
    logits = torch.empty([12, samples, len(keys)])
    ...
    for i, v in enumerate(logs.addition):
        AUPRCs = list()
        for j in range(len(keys)):
            AUPRCs.append(M.average_precision_score(labels[:, j].data.numpy(), logits[i, :, j].data.numpy()))
        logs.result.loc[epoch, v] = np.mean(AUPRCs)

    logits = torch.mean(logits, 0)
    AUPRCs = list()
    for j in range(len(keys)):
        AUPRCs.append(M.average_precision_score(labels[:, j].data.numpy(), logits[:, j].data.numpy()))
...
class Logs(object):
    ...
    def __init__(self, args):
        ...
        self.addition = ['AUPRC{}'.format(i) for i in range(12)]
        self.result = pd.DataFrame(columns=self.inherent + self.addition)
...
if __name__=='__main__':
    ...
    args.save = os.path.join(datapath, 'Ensemble')
